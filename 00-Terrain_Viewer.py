...
def convert_to_vn2000(df_ntd, df_coord):
    ...
    try:
        ...
        # Đồng bộ đảo trục chuẩn đồ họa máy tính để không bị lệch pha với địa chất:
        map_x = dict(zip(df_coord_clean['Cọc_Excel'], df_coord_clean['Y_VN2000'])) # 6 số -> X toán học
        map_y = dict(zip(df_coord_clean['Cọc_Excel'], df_coord_clean['X_VN2000'])) # 7 số -> Y toán học
        
        df_merged = df_ntd_clean.copy()
        df_merged['X_VN2000'] = df_merged['Cọc'].map(map_x)
        df_merged['Y_VN2000'] = df_merged['Cọc'].map(map_y)
        
        # Dự phòng gán tuần tự nếu sai khác ký tự tên cọc
...
def ve_dia_hinh_3d(df, he_so_z=1.0, che_do="Bề mặt mịn", do_min=3):
    """
    🏔️ MÔ HÌNH ĐỊA HÌNH BỀ MẶT LÒNG SÔNG NGUYÊN BẢN (Chuẩn hóa ma trận tọa độ phẳng VN-2000):
    - Trục X nhận dữ liệu X_Real (Dải 6 số), Trục Y nhận Y_Real (Dải 7 số).
    """
