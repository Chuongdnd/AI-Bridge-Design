...
def dap_them_ket_cau_dia_chat_3d(fig, df_hk, df_layers, df_spt, matrix_x, matrix_y, he_so_z=1.0):
    """
    🏗️ HÀM TÍCH HỢP ĐỊA CHẤT 3D CHUẨN HÓA LOGIC KHÔNG GIAN CỦA CHƯƠNG:
    - Đồng bộ tuyệt đối hoán trục đồ họa: X nhận Y_VN2000 (6 số), Y nhận X_VN2000 (7 số).
    - Giải quyết dứt điểm lỗi địa hình một nơi, địa chất một nẻo do ngược trục nội suy.
    - Bo viền đa giác ConvexHull khít kẽ theo chu vi trắc ngang địa hình sông thực tế.
    """
    ...
    # 📐 1. DỰNG ĐA GIÁC ĐƯỜNG BAO ĐỊA HÌNH THEO ĐÚNG HỆ TRỤC MA TRẬN NỀN NTD
    ...
    # Thiết lập lưới nội suy địa chất đồng bộ hoàn toàn dải bao với ma trận ntd
    gx, gy = np.meshgrid(
        np.linspace(float(matrix_x.min()) - 30, float(matrix_x.max()) + 30, 45),
        np.linspace(float(matrix_y.min()) - 30, float(matrix_y.max()) + 30, 45)
    )
    ...
    # 🎯 LUỒNG A: CẮM TRỤ ĐỨNG HỐ KHOAN VÀ ĐƯỜNG SPT (SỬA ĐỒNG BỘ LOGIC TRỤC)
    for _, hk in df_hk_clean.iterrows():
        try:
            ten_hk_goc = str(hk['Ho_Khoan']).strip()
            key_hk = hk['Key_HK']
            
            # ✨ ĐỒNG BỘ LOGIC VỚI NTD: Đồ họa X = Y_Excel (6 số), Đồ họa Y = X_Excel (7 số)
            ...
            # Vẽ các đốt tầng địa chất thẳng đứng ngay tâm hố
            ...
            # Vẽ kết quả thí nghiệm SPT dích dắc bên cạnh hố khoan
            if df_spt is not None:
                ...
                if col_spt and 'Độ sâu thí nghiệm (m)' in df_spt.columns:
                    ...
                    for _, r_spt in df_sub_spt.iterrows():
                        try:
                            ...
                            if nums:
                                d_spt = nums[0]
                                spt_x.append(x_hk + 12.0 + n_val * 0.8) # Đẩy lệch theo trục X đồ họa mới (Y_Excel)
                                spt_y.append(y_hk)
    ...
    # 🎯 LUỒNG B: DỆT MÀNG THẤU KÍNH ĐỊA CHẤT NỘI SUY (ĐỒNG BỘ TRỤC)
    ...
    if len(df_hk_v) >= 2 and not df_layers_clean.empty:
        ...
        for lop_dat in all_unique_layers:
            ...
            # Khởi tạo ma trận cao độ đáy lớp bằng cách nội suy trắc đạc phẳng chuẩn trục
            grid_z = griddata((pt_x, pt_y), pt_z_bot, (gx, gy), method='nearest')
            
            # ✨ CẮT NGẮT KHÔNG GIAN CHUẨN XÁC: Chỉ giữ lại dữ liệu nằm khít trong đa giác địa hình ntd
