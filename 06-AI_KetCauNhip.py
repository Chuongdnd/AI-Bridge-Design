...
# ---------------------------------------------------------------------------
# BEAM_GEOMETRY_DETAIL — thông số hình học chi tiết theo loại dầm và chiều dài
# Đơn vị: mm (trừ L_beam tính bằng mm, S tính bằng mm)
# Cấu trúc: { "Loại dầm": { chiều_dài_m: { ...thông số... } } }
# ---------------------------------------------------------------------------

def _supert_section(S_mm, H_mm, L_mm):
    """Tạo dict mặt cắt Super-T theo khoảng cách dầm S và chiều cao H (mm)."""
    # Outer dimensions derived from S (spacing = top flange width)
    # A-A note: 490+100+1020+100+490=2200 ; bottom=160+700+160=1020
    # Scale from reference S=2200, H=1750
    k  = H_mm / 1750.0
    ks = S_mm / 2200.0
    # Chiều dài đoạn đặc tại đầu dầm (750mm per standard drawing)
    L_dau = 750

    # ── Mặt cắt A-A (giữa nhịp) ──
    # Top flange
    B_ct  = S_mm          # = S_mm (cánh trên rộng bằng khoảng cách dầm)
    H_ct  = round(150 * k)
    # Haunch (vát) cánh trên
    B_vct = round(100 * ks)
    H_vct = round(75 * k)
    # Web outer half-width from center = (S/2 - 490 - 100)
    web_out_hw = round(S_mm / 2 - 490 * ks - 100 * ks)  # ≈510 for S=2200
    # Web thickness: top=110mm, bottom=160mm (from reference)
    B_bung_top  = round(110 * ks)
    B_bung_bot  = round(160 * ks)
    # Web inner half-width
    void_hw_top = web_out_hw - B_bung_top   # ≈400mm
    void_hw_bot = web_out_hw - B_bung_bot   # ≈350mm
    # Bottom flange (B_cd = 2 × web_out_hw = S - 2×490 - 2×100)
    B_cd  = 2 * web_out_hw                  # ≈1020mm for S=2200
    H_cd  = round(225 * k)
    B_vcd = round(50 * k)
    H_vcd = round(50 * k)
    # Web clear height (between top haunch base and bottom flange top)
    H_bung = H_mm - H_ct - H_vct - H_cd - H_vcd
    # Void
    B_rong_tren = 2 * void_hw_top   # ≈800mm (wider at top)
    B_rong_duoi = 2 * void_hw_bot   # ≈700mm (narrower at bottom)
    H_rong      = H_bung

    return {
        "L"  : L_mm,
        "B"  : S_mm + 240,   # total with 120mm overhang each side
        "H"  : H_mm,
        "S"  : S_mm,
        "MC_AA": {
            "B_canh_tren"     : B_ct,
            "H_canh_tren"     : H_ct,
            "B_vat_canh_tren" : B_vct,
            "H_vat_canh_tren" : H_vct,
            "B_bung_top"      : B_bung_top,
            "B_bung_bot"      : B_bung_bot,
            "H_bung"          : H_bung,
            "B_canh_duoi"     : B_cd,
            "H_canh_duoi"     : H_cd,
            "B_vat_canh_duoi" : B_vcd,
            "H_vat_canh_duoi" : H_vcd,
            "web_out_hw"      : web_out_hw,   # outer half-width of web from center
            # Khoang rỗng (void) — hình thang, rộng ở trên, hẹp ở dưới
            "B_rong_tren"     : B_rong_tren,
            "B_rong_duoi"     : B_rong_duoi,
            "H_rong"          : H_rong,
        },
        "MC_BB": {
            "L_dau_dac" : L_dau,
        ...
        "MC_CC": {
            "L_dau_dac"       : L_dau,
            "B_tren"          : B_cd,         # cánh trên thu hẹp = bằng cánh dưới A-A
            "B_duoi"          : B_cd,
            "H"               : H_mm,
            "H_canh_tren_CC"  : 300,
        },
        "cap_DUL": {
            "ong_PVC_D" : 49,
            "so_ong"    : 12 if L_mm >= 35000 else (9 if L_mm >= 28000 else 6),
            "duong_di"  : "parabol",
        },
    }
...
BEAM_GEOMETRY_DETAIL = {
    # ── Super-T ── (BTCT DUL sau, S=2200mm, H=1750mm, ba chiều dài)
    "Super-T": {
        28.2: _supert_section(S_mm=2200, H_mm=1750, L_mm=28200),
        35.7: _supert_section(S_mm=2200, H_mm=1750, L_mm=35700),
        38.2: _supert_section(S_mm=2200, H_mm=1750, L_mm=38200),
    },

    # ── Dầm I ── (BTCT DUL trước hoặc sau, không có khoang rỗng)
    "Dầm I": {
        # L=33m, DUL sau (H=1650mm, S=2250mm)
        33.0: {
            "L": 33000, "B": 650, "H": 1650, "S": 2250,
            "MC_AA": {
                "B_canh_tren": 650, "H_canh_tren": 150,
                "B_vat_canh_tren": 50, "H_vat_canh_tren": 50,
                "B_bung": 200, "H_bung": 1200,
                "B_canh_duoi": 650, "H_canh_duoi": 150,
                "B_vat_canh_duoi": 50, "H_vat_canh_duoi": 50,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 650, "H": 1650, "dac": True},
            "cap_DUL": {"ong_PVC_D": 40, "so_ong": 6, "duong_di": "parabol"},
        },
        # L=20.7m, DUL sau (H=1200mm, S=2250mm)
        20.7: {
            "L": 20700, "B": 600, "H": 1200, "S": 2250,
            "MC_AA": {
                "B_canh_tren": 600, "H_canh_tren": 130,
                "B_vat_canh_tren": 45, "H_vat_canh_tren": 45,
                "B_bung": 180, "H_bung": 890,
                "B_canh_duoi": 600, "H_canh_duoi": 130,
                "B_vat_canh_duoi": 45, "H_vat_canh_duoi": 45,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 600, "H": 1200, "dac": True},
            "cap_DUL": {"ong_PVC_D": 40, "so_ong": 4, "duong_di": "parabol"},
        },
        # L=15m, DUL trước (H=1000mm, S=2250mm)
        15.0: {
            "L": 15000, "B": 450, "H": 1000, "S": 2250,
            "MC_AA": {
                "B_canh_tren": 450, "H_canh_tren": 120,
                "B_vat_canh_tren": 40, "H_vat_canh_tren": 40,
                "B_bung": 160, "H_bung": 720,
                "B_canh_duoi": 450, "H_canh_duoi": 120,
                "B_vat_canh_duoi": 40, "H_vat_canh_duoi": 40,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 450, "H": 1000, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 4, "duong_di": "thang"},
        },
    },

    # ── T ngược ── (cánh dưới rộng, bụng đứng phía trên)
    "T ngược": {
        12.0: {
            "L": 12000, "B": 980, "H": 550, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 200, "H_canh_tren": 350,   # bụng = "cánh trên" của T ngược
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 200, "H_bung": 350,
                "B_canh_duoi": 980, "H_canh_duoi": 200,   # cánh dưới rộng
                "B_vat_canh_duoi": 30, "H_vat_canh_duoi": 30,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 980, "H": 550, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 2, "duong_di": "thang"},
        },
        15.0: {
            "L": 15000, "B": 980, "H": 550, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 200, "H_canh_tren": 350,
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 200, "H_bung": 350,
                "B_canh_duoi": 980, "H_canh_duoi": 200,
                "B_vat_canh_duoi": 30, "H_vat_canh_duoi": 30,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 980, "H": 550, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 2, "duong_di": "thang"},
        ...
        18.0: {
            "L": 18000, "B": 980, "H": 750, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 200, "H_canh_tren": 500,
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 200, "H_bung": 500,
                "B_canh_duoi": 980, "H_canh_duoi": 250,
                "B_vat_canh_duoi": 30, "H_vat_canh_duoi": 30,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 980, "H": 750, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 3, "duong_di": "thang"},
        },
    },

    # ── Dầm bản rỗng ── (mặt trên+dưới phẳng, khoang rỗng tròn bên trong)
    "Dầm bản rỗng": {
        12.0: {
            "L": 12000, "B": 990, "H": 500, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 990, "H_canh_tren": 80,   # bản mặt trên
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 990, "H_bung": 340,            # toàn tiết diện bản
                "B_canh_duoi": 990, "H_canh_duoi": 80,
                "B_vat_canh_duoi": 0, "H_vat_canh_duoi": 0,
                # Khoang rỗng tròn: D=300mm, số lượng, khoảng cách tim
                "B_rong": 300,       # đường kính lỗ rỗng
                "H_rong": 300,
                "so_rong": 3,        # số lỗ rỗng
                "kc_rong": 330,      # khoảng cách tim lỗ rỗng
                "y_rong": 250,       # vị trí tâm lỗ từ đáy dầm
            },
            "MC_BB": {"L_dau_dac": 0, "B": 990, "H": 500, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 4, "duong_di": "thang"},
        },
        18.0: {
            "L": 18000, "B": 990, "H": 650, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 990, "H_canh_tren": 100,
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 990, "H_bung": 450,
                "B_canh_duoi": 990, "H_canh_duoi": 100,
                "B_vat_canh_duoi": 0, "H_vat_canh_duoi": 0,
                "B_rong": 370, "H_rong": 370,
                "so_rong": 3, "kc_rong": 330, "y_rong": 325,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 990, "H": 650, "dac": True},
            "cap_DUL": {"ong_PVC_D": 40, "so_ong": 4, "duong_di": "thang"},
        },
        24.0: {
            "L": 24000, "B": 990, "H": 950, "S": 1000,
            "MC_AA": {
                "B_canh_tren": 990, "H_canh_tren": 120,
                "B_vat_canh_tren": 0, "H_vat_canh_tren": 0,
                "B_bung": 990, "H_bung": 710,
                "B_canh_duoi": 990, "H_canh_duoi": 120,
                "B_vat_canh_duoi": 0, "H_vat_canh_duoi": 0,
                "B_rong": 450, "H_rong": 450,
                "so_rong": 3, "kc_rong": 330, "y_rong": 475,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 990, "H": 950, "dac": True},
            "cap_DUL": {"ong_PVC_D": 40, "so_ong": 6, "duong_di": "thang"},
        },
    },

    # ── Dầm T ── (cánh trên rộng, bụng đứng phía dưới)
    "Dầm T": {
        12.0: {
            "L": 12000, "B": 1800, "H": 900, "S": 2400,
            "MC_AA": {
                "B_canh_tren": 1800, "H_canh_tren": 200,
                "B_vat_canh_tren": 80, "H_vat_canh_tren": 80,
                "B_bung": 300, "H_bung": 700,
                "B_canh_duoi": 300, "H_canh_duoi": 0,   # không có cánh dưới riêng
                "B_vat_canh_duoi": 0, "H_vat_canh_duoi": 0,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 1800, "H": 900, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 4, "duong_di": "parabol"},
        },
        15.0: {
            "L": 15000, "B": 1800, "H": 1000, "S": 2400,
            "MC_AA": {
                "B_canh_tren": 1800, "H_canh_tren": 200,
                "B_vat_canh_tren": 80, "H_vat_canh_tren": 80,
                "B_bung": 300, "H_bung": 800,
                "B_canh_duoi": 300, "H_canh_duoi": 0,
                "B_vat_canh_duoi": 0, "H_vat_canh_duoi": 0,
            },
            "MC_BB": {"L_dau_dac": 0, "B": 1800, "H": 1000, "dac": True},
            "cap_DUL": {"ong_PVC_D": 32, "so_ong": 4, "duong_di": "parabol"},
        },
    },
}
...
def get_beam_geometry_detail(loai_dam: str, chieu_dai_m: float):
    """
    Trả về dict thông số hình học chi tiết cho loại dầm và chiều dài.

    Parameters
    ----------
    loai_dam    : str   — tên loại dầm ("Super-T", "Dầm I", "T ngược", ...)
    chieu_dai_m : float — chiều dài nhịp tính bằng MÉT

    Returns
    -------
    dict hoặc None nếu không tìm thấy trong catalog.
    Tất cả kích thước trong dict trả về tính bằng mm.
    """
    entry = BEAM_GEOMETRY_DETAIL.get(loai_dam)
    if entry is None:
        return None
    # Tìm chiều dài gần nhất trong catalog (dung sai ±1m)
    best_L = min(entry.keys(), key=lambda k: abs(k - chieu_dai_m))
    if abs(best_L - chieu_dai_m) > 1.0:
        return None
    return entry[best_L]
...
def predict_kcn(B_tk, H_tk, goc, B_cau, moi_truong,
    ...
    Mỗi phương án có thêm key "geometry_detail" (dict hoặc None) lấy từ
    BEAM_GEOMETRY_DETAIL — dùng bởi module vẽ mặt cắt chi tiết.
    """
    def _enrich(pa):
        pa["geometry_detail"] = get_beam_geometry_detail(
            pa["loai_dam"], pa["chieu_dai"]
        )
        return pa

    return {
        "pa1_chi_phi": _enrich(_predict_rb_chi_phi(B_tk, goc, B_cau, L_cau_tong)),
        "pa2_my_quan": _enrich(_predict_rb_my_quan(B_tk, goc, B_cau, L_cau_tong)),
        "pa3_ai":      _enrich(_predict_ai(B_tk, H_tk, goc, B_cau, moi_truong,
                                           L_cau_tong, models)),
    }
