...
def ve_trac_doc_cau(res):
    ...
    # 1. KHUNG HÌNH CỐ ĐỊNH 120m
    x_min_view, x_max_view = -5, 125
    x_draw = np.linspace(0, 120, 200)

    # 2. VẼ ĐỊA HÌNH
    if is_duong_bo:
        # Vượt đường: Nét liền xám (Mặt đường bị vượt)
        ...
        # Vượt sông: Nét đứt xanh lá (Đường tự nhiên trung bình)
        y_tn = np.full_like(x_draw, h_tn_tb)
        ax.plot(x_draw, y_tn, color='#27ae60', ls='--', lw=2.0)
    ...
    if B > 0:
        ...
        ax.add_patch(rect)
        # DIM B & H (giữ nguyên logic tọa độ cố định như bạn muốn)
        y_dim_b = h5 + H_tk + 1.0
