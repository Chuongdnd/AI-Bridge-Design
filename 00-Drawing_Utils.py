...
def ve_trac_doc_cau(res):
  ...
  # --- 2. VẼ ĐỊA HÌNH VÀ ĐƯỜNG TỰ NHIÊN TB ---
    ...
    if geo:
        ...
        ax.text(x_center, geo['y_dinh'] + 1.2, f"TỔNG CHIỀU DÀI CẦU: L = {geo['L_cau']:.2f}m", 
                color='brown', fontweight='bold', ha='center', 
                bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))
        
        # Hiển thị thông số Đường đỏ (i và R)
        ax.text(x_center, geo['y_dinh'] + 0.5, f"ĐƯỜNG ĐỎ: i_max={geo['i_val']*100}% | R={geo['R']}m", 
        ...
        ax.text(x_center, y_day_tim - 0.8, f"CAO ĐỘ ĐÁY DẦM TẠI TIM: {y_day_tim:.3f}m", 
                color='#34495e', fontsize=9, ha='center', fontweight='bold')
    else:
        # Thông báo nếu dữ liệu geo_logic chưa được truyền từ Interface sang
        ax.text(x_center, h_dam + 1, "⚠️ CHỜ DỮ LIỆU HÌNH HỌC TỪ FILE 02...", 
    ...
    if B > 0:
        ...
        rect = patches.Rectangle((x_center - B/2, h5), B, H_tk, fill=False, edgecolor='magenta', ls='--', lw=2.5, zorder=3)
        ax.add_patch(rect)
        ...
        y_dim_b = h5 + H_tk/2
        ax.annotate('', xy=(x_center + B/2, y_dim_b), xytext=(x_center - B/2, y_dim_b),
                    arrowprops=dict(arrowstyle='<->', color='black', lw=1.2, mutation_scale=15))
        
        # Hiển thị text với 2 dòng nếu có góc xiên
        ax.text(x_center, y_dim_b + 0.3, label_B, ha='center', va='bottom', 
                fontweight='bold', fontsize=9, 
                bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))

        # 5.4. DIM Chiều cao H
        ax.annotate('', xy=(x_center + B/2 + 3, h5 + H_tk), xytext=(x_center + B/2 + 3, h5),
                    arrowprops=dict(arrowstyle='<->', color='black', lw=1.2, mutation_scale=15))
        ax.text(x_center + B/2 + 3.5, h5 + H_tk/2, f"H = {H_tk}m", ha='left', va='center', 
                rotation=90, fontweight='bold', 
                bbox=dict(facecolor='white', alpha=0.7, edgecolor='none'))
        # --- 5.5. FIX LỖI CHỒNG LẤN NHÃN DIM CẤU TẠO ---
        x_dim_cau_tao = x_center - 30
