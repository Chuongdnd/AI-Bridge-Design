...
def _extrude_profile(xs_2d, ys_2d, x_start, x_end, color, name, opacity=0.92):
    """
    Kéo dài profile 2D (ys_2d là y, xs_2d là z) theo trục x (chiều dài dầm).
    Trả về go.Mesh3d trace.

    Hệ trục: x=chiều dài dầm, y=bề rộng ngang, z=chiều cao.
    xs_2d → z-axis (chiều cao)
    ys_2d → y-axis (bề rộng)
    """
    n = len(xs_2d)
    # Đầu dầm (x = x_start)
    v_x0 = [x_start] * n
    v_y0 = list(xs_2d)  # xs_2d là bề rộng y
    v_z0 = list(ys_2d)  # ys_2d là chiều cao z
    ...
    # Tam giác: 2 mặt đầu + mặt bên xung quanh
    ii, jj, kk = [], [], []

    # Mặt bên (side faces) — pair of triangles per edge
    for i in range(n):
        i_next = (i + 1) % n
        a, b, c, e = i, i_next, i + n, i_next + n
        ii += [a, a]; jj += [b, c]; kk += [c, e]

    # Mặt đầu bên trái (fan triangulation from vertex 0)
    for i in range(1, n - 1):
        ii.append(0); jj.append(i); kk.append(i + 1)

    # Mặt đầu bên phải (offset by n)
    for i in range(1, n - 1):
...
def ve_chi_tiet_3d(d):
    ...
    # Gối dầm (bearing pads — hình hộp nhỏ mỗi đầu)
    L_goi = min(0.75, L * 0.02)
    for i, yc in enumerate(beam_positions):
        hw = max(0.25, H * 0.20)
        for xp in [0.0, L - L_goi]:
            traces.append(go.Mesh3d(
                x=[xp, xp+L_goi, xp+L_goi, xp, xp, xp+L_goi, xp+L_goi, xp],
                y=[yc-hw, yc-hw, yc+hw, yc+hw, yc-hw, yc-hw, yc+hw, yc+hw],
                z=[-H-0.06, -H-0.06, -H-0.06, -H-0.06, -H, -H, -H, -H],
                i=[0,0,4,4,0,0,3,3,0,0,1,1],
                j=[1,2,5,6,1,5,2,6,3,7,2,6],
                k=[2,3,6,7,5,4,6,7,7,4,6,5],
                color="#c0a06b", opacity=0.9,
                name="Gối dầm" if (i == 0 and xp == 0.0) else "",
                showlegend=(i == 0 and xp == 0.0),
            ))
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            yaxis_title="Bề rộng ngang (m)",
            zaxis_title="Chiều cao (m)",
            aspectmode="data",
...
# ─────────────────────────────────────────────────────────────────────────────
# 6. RENDER TOÀN BỘ TAB CHI TIẾT DẦM
# ─────────────────────────────────────────────────────────────────────────────

def render_chi_tiet_dam_tab(d, st):
    """
    Render tab Chi tiết dầm với đầy đủ bản vẽ.
    Gọi từ 00-Interface.py: CTD.render_chi_tiet_dam_tab(d, st)
    """
    kcn  = d.get("kcn_result") or d.get("ai_result", {})
    loai = str(kcn.get("loai_dam", "Super-T"))
    H    = float(kcn.get("chieu_cao_dam") or kcn.get("chieu_cao", 1.75))
    ...
    st.markdown(f"##### Chi tiết dầm {loai.upper()} — L={L:.1f}m | H={H*1000:.0f}mm | {n_dam} dầm/MCN")
    st.caption(
        f"Khoảng cách tim: {kc*1000:.0f}mm | "
        f"Tỷ lệ cánh trên: {kc*1000:.0f}mm | "
        f"Tỷ lệ L/H = {L/H:.1f}"
    )

    # ── Thông số dầm ────────────────────────────────────────────────────
    with st.expander("📐 Thông số hình học dầm", expanded=False):
        ll = loai.lower()
        if "super" in ll:
            dd = _spt_dims(H, kc)
            st.table({
                "Thông số": [
                    ...
                    "Chiều cao dầm H",
                    "Cánh trên (tổng, kc)",
                    "Cánh trên (trong: 225+770+225)",
                    "Chiều dày cánh trên",
                    "Web (sườn)",
                    "Cánh đáy (160+700+160)",
                    "Chiều dày cánh đáy",
                    "Tỷ lệ L/H",
                ],
                "Giá trị": [
                    f"{L:.2f} m = {L*1000:.0f} mm",
                    f"{H:.3f} m = {H*1000:.0f} mm",
                    f"{kc:.3f} m = {kc*1000:.0f} mm",
                    f"{dd['tin_hw']*2:.3f} m = {dd['tin_hw']*2000:.0f} mm",
                    f"{dd['tf_h']:.3f} m = {dd['tf_h']*1000:.0f} mm",
                    f"{dd['w_hw']*2:.3f} m = {dd['w_hw']*2000:.0f} mm",
                    f"{dd['bf_hw']*2:.3f} m = {dd['bf_hw']*2000:.0f} mm",
                    f"{dd['bf_h']:.3f} m = {dd['bf_h']*1000:.0f} mm",
                    f"{L/H:.1f} (tối ưu 18–22 cho Super-T)",
                ],
            })
        elif "t ngược" in ll or "tngược" in ll:
            dd = _tngược_dims(H, kc)
            st.table({
                "Thông số": ["Chiều dài L", "Chiều cao H", "Web trên", "Cánh đáy", "Tỷ lệ L/H"],
                "Giá trị": [
                    f"{L:.2f} m", f"{H:.3f} m",
                    f"{dd['w_hw']*2*1000:.0f} mm",
                    f"{dd['bf_hw']*2*1000:.0f} mm",
                    f"{L/H:.1f}",
                ],
            })
        else:  # Dầm I
            dd = _dami_dims(H, kc)
            st.table({
                "Thông số": ["Chiều dài L", "Chiều cao H", "Cánh I", "Web I", "Tỷ lệ L/H"],
                "Giá trị": [
                    f"{L:.2f} m", f"{H:.3f} m",
                    f"{dd['fw']*2*1000:.0f} mm",
                    f"{dd['tw']*2*1000:.0f} mm",
                    f"{L/H:.1f}",
                ],
            })

    # ── MCN đầu dầm + giữa dầm ──────────────────────────────────────────
    st.markdown("**① Mặt cắt ngang — MCT A-A (đầu dầm) và MCT B-B (giữa dầm)**")
    try:
        fig_mcn = ve_chi_tiet_mcn(d)
        st.plotly_chart(fig_mcn, use_container_width=True,
                        config={"scrollZoom": True, "displayModeBar": True},
                        key="ctd_mcn")
    except Exception as e:
        st.error(f"Lỗi vẽ MCN chi tiết: {e}")

    # ── Mặt cắt dọc ──────────────────────────────────────────────────────
    st.markdown("**② Mặt cắt dọc dầm — Tim dầm (MCD)**")
    try:
        fig_doc = ve_chi_tiet_mat_cat_doc(d)
        st.plotly_chart(fig_doc, use_container_width=True,
                        config={"scrollZoom": True, "displayModeBar": True},
                        key="ctd_doc")
    except Exception as e:
        st.error(f"Lỗi vẽ mặt cắt dọc: {e}")

    # ── Mặt bằng dầm ─────────────────────────────────────────────────────
    st.markdown("**③ Mặt bằng dầm — Nhìn từ trên (MB)**")
    try:
        fig_mb = ve_chi_tiet_mat_bang(d)
        st.plotly_chart(fig_mb, use_container_width=True,
                        config={"scrollZoom": True, "displayModeBar": True},
                        key="ctd_mb")
    except Exception as e:
        st.error(f"Lỗi vẽ mặt bằng: {e}")

    # ── 3D dầm ───────────────────────────────────────────────────────────
    st.markdown("**④ Mô hình 3D nhịp dầm**")
    try:
        fig_3d = ve_chi_tiet_3d(d)
        st.plotly_chart(fig_3d, use_container_width=True,
                        config={"scrollZoom": True, "displayModeBar": True},
                        key="ctd_3d")
        st.caption("Kéo chuột để xoay · Scroll để zoom · Shift+drag để pan")
    except Exception as e:
        st.error(f"Lỗi vẽ 3D dầm: {e}")
