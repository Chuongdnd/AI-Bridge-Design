...
with tab1:
    ...
    with col_main:
        ...
        if st.button("💬 " + ("Đóng Trợ lý" if st.session_state.show_chat else "Mở Trợ lý AI"), key="btn_chat_t1"):
...
if col_chat_area:
    with col_chat_area:
        ...
        # Ô nhập liệu (Phải nằm trong with col_chat_area)
        if prompt := st.chat_input("Hỏi tôi về thiết kế cầu...", key="chat_input_v2"):
            # 1. Hiển thị tin nhắn người dùng ngay lập tức
            st.session_state.messages.append({"role": "user", "content": prompt})
            
            # 2. Xử lý AI
            try:
                # Kiểm tra xem có dữ liệu module chưa
                design_info = st.session_state.get('design_data', "Chưa có dữ liệu thiết kế cụ thể.")
                
                # Tạo bối cảnh (System Message)
                system_message = f"""
                ...
                # 3. Lưu kết quả của AI vào lịch sử
                st.session_state.messages.append({"role": "assistant", "content": response.text})
                
                # Làm mới trang để tin nhắn hiện lên
                st.rerun()
                
            except Exception as e:
                if "429" in str(e):
                    st.error("⚠️ Bạn hỏi quá nhanh hoặc tài liệu quá lớn. Đợi 60s nhé!")
                else:
                    st.error(f"Lỗi AI: {e}")
